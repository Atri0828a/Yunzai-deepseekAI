...
2.执行完依赖安装命令之后，重启依然显示缺依赖：这不是我的问题喵~ 自己想办法解决喵~ （可能是nodejs的问题吧）  
3.消息包含关键词却无法触发AI对话：检查你的触发关键词是否和你在云崽自己的配置文件中的别称相同，云崽会把别称裁掉再把消息传入，导致无法触发  
...
直接在GitHub或者gitee提issues我不一定能及时看见，建议直接联系上面的QQ或者发邮箱（atri0828a@gmail.com）
